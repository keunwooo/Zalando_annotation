...
func NewController(controllerConfig *spec.ControllerConfig, controllerId string) *Controller {
	...
	//eventBroadcasterImpl 반환 내부에 브로드캐스터 변수
	//NewBroadcaster는 새로운 브로드캐스터를 생성합니다. queueLength는 감시자당 대기열에 넣을 최대 이벤트 수입니다.
	//이벤트가 발생하는 순서대로 배포되는 것이 보장되지만 하나의 이벤트가 모든 Watcher에게 배포되는 순서는 지정되지 않습니다.
	...
	//NewRecorder는 인터페이스를 래핑하고 인터페이스를 통해 전송된 모든 변경 사항을 기록합니다.
