...
func RestConfig(kubeConfig string, outOfCluster bool) (*rest.Config, error) {
	if outOfCluster {
		...
		//BuildConfigFromFlags는 마스터 URL 또는 kubeconfig 파일 경로에서 구성을 빌드하는 도우미 함수입니다. 이는 클러스터 구성 요소에 대한 명령줄 플래그로 전달됩니다.
		// 경고는 이 사용법을 반영해야 합니다. masterUrl 또는 kubeconfigPath가 전달되지 않으면 inClusterConfig로 대체합니다. inClusterConfig가 실패하면 기본 구성으로 대체합니다.
	}

	return rest.InClusterConfig()
	//InClusterConfig는 kubernetes가 포드에 제공하는 서비스 계정을 사용하는 구성 개체를 반환합니다.
	//kubernetes에서 실행되는 포드 내에서 실행될 것으로 예상되는 클라이언트를 위한 것입니다. kubernetes 환경에서 실행되지 않는 프로세스에서 호출되면 ErrNotInCluster를 반환합니다.
}
...
func NewFromConfig(cfg *rest.Config) (KubernetesClient, error) {
	...
	//NewForConfig는 지정된 구성에 대한 새 Clientset을 만듭니다.
	//구성의 RateLimiter가 설정되지 않고 QPS 및 버스트가 허용되는 경우 NewForConfig는 configShallowCopy에서 속도 제한기를 생성합니다.
	// NewForConfig는 NewForConfigAndClient(c, httpClient)와 동일하며, 여기서 httpClient는 rest.HTTPClientFor(c)로 생성됩니다.
